...
function SingupComponent(props) {
      ...
      e.preventDefault();
         ...
         let 아이디중복확인 = false;
            ...
            if(regExp1.test(아이디)===false || regExp2.test(아이디)===false || regExp3.test(아이디)===true || regExp4.test(아이디)===true ){                  
                  ...
                  axios({
                     url:'https://moonjong.co.kr/week_kurl_cra4_cors/id_select.php',
                     mothod:'GET'
                  })
                  .then((res)=>{
   ...
   const onClickEmail=(e)=>{
      ...
      if(emailVal===''){
         ...
         txt = `이메일 형식으로 입력해 주세요.`;
               ...
               // 리액트 axios() 구현
               axios({
                  url:'https://moonjong.co.kr/week_kurl_cra4_cors/email_select.php',
                  method: 'GET'
               })
               .then((res)=>{
   ...
   const onClickSumbmit=(e)=>{
      e.preventDefault();
         ...
         // // 22-4. 폼데이터 axios 패키지 이용 데이터베이스 서벙에 전송 저장
         axios({
            url:'https://moonjong.co.kr/week_kurl_cra4_cors/member_kurly.php',
            method:'POST',
            data: newFormData
         })
         .then((res)=>{
